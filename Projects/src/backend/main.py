...
async def callback(
    ...
    if is_ext:
        ...
        # delivery_type은 기본값("email")이 있으므로 온보딩 완료 판단에서 제외
        is_onboarded = bool(user.initial_intent or user.interest_categories)
        redirect_url = f"{FRONTEND_URL}/loading.html?token={jwt_token}" if not is_onboarded else f"{FRONTEND_URL}/mypage.html?token={jwt_token}"
